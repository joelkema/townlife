...
const eat = (citizen: Citizen): Citizen => ({
    ...
    basicNeeds: {
        ...citizen.basicNeeds,
        hunger: citizen.basicNeeds.hunger - 50000,
    },
});
...
const townlife = (state: AppState) => {
    const tick = (tick: number): AppState => {
        ...
        const citizens = Object.keys(state.citizens).reduce(
            (map: Record<string, Citizen>, id) => {
                ...
                const citizen = pipe(
                    ...
                    // not sleeping
                    when(
                        and(not(isSleeping), shouldChangeRest(tick)),
                        decreaseRest
                    ),
